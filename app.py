...
async def search():
    ...
    if query == 'популярные':
        popular_prompt = """Сгенерируй 3 популярных ИИ-сервиса в формате JSON. 
        ...
        - icon (уникальная SVG иконка, соответствующая категории сервиса, в формате строки, размером 64x64, только используй stroke="currentColor" и fill="none")
    ...
    prompt = f"""Сгенерируй до 5 ИИ-сервисов, соответствующих запросу "{query}", в формате JSON.
    Каждый сервис должен содержать поля:
    - id (число)
    - name (название сервиса)
    - category (категория, например "Генерация изображений")
    - description (краткое описание сервиса, 1-2 предложения)
    - icon (SVG иконка, соответствующая категории сервиса, в формате строки, размером 64x64, только используй stroke="currentColor" и fill="none")
    - tags (массив ключевых слов для поиска)
    - pricing (строка, одно из: "free", "paid", "freemium")
    - url (рабочая ссылка на официальный сайт сервиса)
